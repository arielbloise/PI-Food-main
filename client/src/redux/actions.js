...
export const getAllRecipes = () => {
  return function(dispatch) {
    fetch(('http://localhost:3001/recipes'))
      ...
      .then(data => {
        const dbRecipes = data.filter(recipe => recipe.recetaId); // Filtrar las recetas que provienen de la base de datos
        const apiRecipes = data.filter(recipe => !recipe.recetaId); // Filtrar las recetas que provienen de la API
